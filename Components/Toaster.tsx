...
const Toaster = ({  w = '14em', ...props }: ToasterProps) => {
  const dispatch = useDispatch();
  const [cookedGroup, setCookedGroup] = useState<CookedGroup>();
  const [status, setStatus] = useState<string | null>(null);
  const [move, setMove] = useState<boolean | undefined>(undefined);
  const [haveOverCook, setHaveOverCook] = useState<boolean | undefined>(undefined);
        ...
        {status ? (
          ...
          >
            <Image
              ...
              pointerEvents="none"
              cursor={(isDone || isOver) ? 'pointer' : undefined}
              userSelect="none"
              w={w}
              alt="toaster"
            />
